...
import { bookingLayananTambahan, getSlotBooking, SlotBooking } from "@/app/lib/pesanan";
...
export default function FormLayananTambahan({ layanan }: Props) {
    const router = useRouter();

    const [tanggalPesanan, setTanggalPesanan] = useState("");
    const [jamPesanan, setJamPesanan] = useState("");
    const [areaWaxing, setAreaWaxing] = useState<string[]>([]);
    ...
    const [errorAreaWaxing, setErrorAreaWaxing] = useState("");

    const pilihanAreaWaxing = ["Ketiak", "Tangan", "Kaki"];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (layanan.kategori_layanan === "waxing" && areaWaxing.length === 0) {
            setErrorAreaWaxing("Silakan pilih area waxing");
            return;
        }
        ...
        if (layanan.kategori_layanan === "waxing") {
            formData.append("area_waxing", areaWaxing.join(", "));
        }

        const result = await bookingLayananTambahan(formData);

        router.push(`/page/pemesanan/pembayaran?id=${result.data.id_pesanan}`);
        } catch (error) {
        console.error("Booking layanan gagal:", error);
        } finally {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5 md:px-7">
            <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {errorJam && (
                        <p className="text-[11px] text-red-500">{errorJam}</p>
                    )}
                </div>
            </div>
            ...
            {layanan.kategori_layanan === "waxing" && (
                <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {pilihanAreaWaxing.map((item) => (
                            ...
                            >
                                <input
                                    type="checkbox"
                                    checked={areaWaxing.includes(item)}
                                    onChange={() => {
                                        handleCheckboxChange(item, areaWaxing, setAreaWaxing);
                                        setErrorAreaWaxing("");
                                    }}
                    ...
                    {errorAreaWaxing && (
                        <p className="text-[11px] text-red-500">{errorAreaWaxing}</p>
                    )}
                </div>
            )}
